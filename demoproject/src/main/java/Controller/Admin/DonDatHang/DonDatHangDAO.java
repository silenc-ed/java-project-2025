...
public class DonDatHangDAO {
    ...
    public int createOrder(Integer maKH, int maNV, int maCN,
        ...
        try {
            ...
            // 1. INSERT HOADON
            String sqlHD = "INSERT INTO HOADON (MA_KH, MA_NV, MA_CN, TONG_TIEN_HANG, GIAM_GIA, THANH_TIEN, TRANG_THAI) " +
            ...
            if (serials != null && !serials.isEmpty()) {
                String sqlCT = "INSERT INTO CHITIET_HOADON (MA_HD, MA_SP, SERIAL_NUMBER, SO_LUONG, DON_GIA, THANH_TIEN) " +
                               "VALUES (?, ?, ?, 1, ?, ?)";
            ...
            String sqlUpdate = "UPDATE HOADON SET TONG_TIEN_HANG = ?, THANH_TIEN = ? WHERE MA_HD = ?";
    ...
    public void updateOrder(int maHD, Integer maKH,
        ...
        try {
            ...
            // Kiểm tra trạng thái
            String checkSql = "SELECT TRANG_THAI FROM HOADON WHERE MA_HD = ?";
            ...
            String sqlReleaseOld = "UPDATE KHO_SERIAL SET TRANG_THAI = N'KHA_DUNG' " +
                                   "WHERE SERIAL_NUMBER IN (SELECT SERIAL_NUMBER FROM CHITIET_HOADON WHERE MA_HD = ? AND SERIAL_NUMBER IS NOT NULL)";
            ...
            // Xóa chi tiết cũ
            try (PreparedStatement psDel = con.prepareStatement("DELETE FROM CHITIET_HOADON WHERE MA_HD = ?")) {
                psDel.setInt(1, maHD);
                psDel.executeUpdate();
            }
            ...
            long giamGia = 0;
            String sqlGetKM = "SELECT MA_KM FROM HOADON WHERE MA_HD = ?";
            ...
            String sqlUpd = "UPDATE HOADON SET MA_KH = ?, TONG_TIEN_HANG = ?, GIAM_GIA = ?, THANH_TIEN = ? WHERE MA_HD = ?";
    ...
    public void cancelOrder(int maHD, String lyDoHuy) throws Exception {
        ...
        try {
            ...
            String sqlCancel = "UPDATE HOADON SET TRANG_THAI = N'Đã hủy', LY_DO_HUY = ? WHERE MA_HD = ?";
    ...
    public List<Map<String, Object>> searchOrders(String keyword, String trangThai,
        ...
        sql.append("FROM HOADON H LEFT JOIN KHACHHANG K ON H.MA_KH = K.MA_KH ");
    ...
    public List<Map<String, Object>> getOrderProducts(int maHD) throws Exception {
        ...
        String sql = "SELECT CT.MA_CTHD, CT.MA_SP, SP.TEN_SP, CT.SERIAL_NUMBER, CT.DON_GIA, CT.THANH_TIEN " +
                     "FROM CHITIET_HOADON CT JOIN SANPHAM SP ON CT.MA_SP = SP.MA_SP " +
                     "WHERE CT.MA_HD = ?";
        try (Connection con = ConnectionUtils.getMyConnection();
    ...
    public long applyPromotion(int maHD, int maKM) throws Exception {
        try (Connection con = ConnectionUtils.getMyConnection()) {
            // Lấy tổng tiền hàng
            long tongTien = 0;
            try (PreparedStatement ps = con.prepareStatement("SELECT TONG_TIEN_HANG FROM HOADON WHERE MA_HD = ?")) {
                ps.setInt(1, maHD);
                try (ResultSet rs = ps.executeQuery()) {
            ...
            // Cập nhật HOADON
            String sqlUpd = "UPDATE HOADON SET MA_KM = ?, GIAM_GIA = ?, THANH_TIEN = ? WHERE MA_HD = ?";
    ...
    public long validateAndCalculateDiscount(Connection con, int maKM, long tongTien) throws Exception {
        String sql = "SELECT GIA_TRI, RANG_BUOC_GIA_TRI, NGAY_BAT_DAU, NGAY_KET_THUC, TRANG_THAI FROM KHUYENMAI WHERE MA_KM = ?";
    ...
    public boolean isSerialInPendingOrder(String serialNumber, int excludeHD) throws Exception {
        String sql = "SELECT COUNT(*) FROM CHITIET_HOADON CT " +
    ...
    public List<Map<String, Object>> getAvailableServices() throws Exception {
        List<Map<String, Object>> results = new ArrayList<>();
        String sql = "SELECT MA_DV, TEN_DV, MO_TA, GIA_CUOC FROM DICHVU WHERE TRANG_THAI = 1";
        try (Connection con = ConnectionUtils.getMyConnection();
             PreparedStatement ps = con.prepareStatement(sql);
    ...
    public String getOrderStatus(int maHD) throws Exception {
        try (Connection con = ConnectionUtils.getMyConnection();
             PreparedStatement ps = con.prepareStatement("SELECT TRANG_THAI FROM HOADON WHERE MA_HD = ?")) {
            ps.setInt(1, maHD);
            try (ResultSet rs = ps.executeQuery()) {
